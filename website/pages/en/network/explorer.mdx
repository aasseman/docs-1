...
Learn about the Graph Explorer, your decentralized access point to subgraphs and network data.

In Graph Explorer you can interact with other subgraph developers, dapp developers, Curators, Indexers and Delegators.

For a general overview of Graph Explorer, check out the video below or read the overview:

<VideoEmbed youtube="u224xf7rEBY" />

## Overview

After you deploy and publish your subgraph in [Subgraph Studio](https://thegraph.com/studio/), you can view both your subgraph and subgraphs published by others on the Subgraphs tab.

You can find the exact subgraph you’re looking for based on the date created, signal amount, or name.

![Explorer Image 1](/img/Subgraphs-Explorer-Landing.png)

When you click into a subgraph, you’ll be able to do the following:

- Test queries in the staging playground of Subgraph Studio and leverage network details to make informed decisions.
- [Signal GRT](/publishing/publishing-a-subgraph/#adding-signal-to-your-subgraph) on any subgraph to draw Indexers to it. It's crucial to signal on a subgraph because it incentivizes indexing, ensuring it surfaces on the network to serve queries.

![Explorer Image 2](/img/Subgraph-Details.png)

On each subgraph's dedicated page, you can view and perform the following actions:
...
- Test subgraphs in the staging playground
- View the Indexers that are indexing on a certain subgraph
- View subgraph stats (allocations, Curators, etc)
...
In the subgraph’s tab, you’ll see an overview of all network participants, including Indexers, Delegators, and Curators.

### 1. Indexers

![Explorer Image 4](/img/Indexer-Pane.png)

Indexers are integral to the protocol as they stake on subgraphs, index them, and respond to queries from users consuming subgraphs. In the Indexers table, you can view an Indexer's delegation parameters, their stake amount, their stakes for each subgraph, and the revenue they have generated from query fees and indexing rewards. See specifics below:
...
Your personal profile is the place for you to see your network activity, no matter how you’re participating on the network. Your crypto wallet will act as your user profile, and with the User Dashboard, you’ll be able to see:
